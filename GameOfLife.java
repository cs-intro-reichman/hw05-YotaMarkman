...
public class GameOfLife 
{
	...
	public static void test2(String fileName) 
	{
		...
		int[][] board = read(fileName);
		count(board, i, j);
		int newnum = (cellValue(board, i, j));
	...
	private static int[][] evolve(int[][] board) 
	{
		for ( int i = 0; i < board.length - 2; i++)
		{
			...
			{
				int evolvedcell = cellValue(board, i, j);
				board[i][j] = evolvedcell;
			}
		}
		return board;
	}
			...
			for ( int l = 1; l < board[k].length ; l++)
			{
				if ( k == i && l == j)
				{
					if ( board[k][l] == 1)
					{
						...
						else if ( board[k-1][l-1] == 1)
						{
							countalive++;
						}
						else if ( board[k][l-1] == 1)
						{
							countalive++;
						}
						else if ( board[k+1][l-1] == 1)
